...
type Props = {
  ...
  onHintUsed?: () => void;
  onIndependentSubmit?: (opts: { responseLatencyMs: number; answerChanged: boolean }) => void;
  autoPlayNarration?: boolean;
  onNarrationReplay?: () => void;
};

export function IndependentTryScene({
  displayText,
  voiceoverText,
  expectedAnswer,
  hints = [],
  ...
  function handleSubmit() {
    ...
    onIndependentSubmit?.({ responseLatencyMs, answerChanged: answerChangedRef.current });
    stopNarration();
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm text-[rgb(var(--learner-text-muted))]">
        Try this one on your own. Type your answer below.
      </p>
      ...
      {hints.length > 0 && (
        <div className="mt-4">
          ...
          >
            {hintIndex < 0 ? 'I need a hint' : hintIndex < hints.length - 1 ? 'Another hint' : 'Hints'}
          </button>
          {hintIndex >= 0 && (
            <motion.p
